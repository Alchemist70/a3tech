...
const AdminJambTopicDetailsTab: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const [sectionsRes, topicsRes, detailsRes] = await Promise.all([
          ...
          api.get('/jamb-topic-details', { withCredentials: true })
        ]);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const res = await api.post('/jamb-topic-details', payload, { withCredentials: true });
      const created = res.data;
      if (!created) {
        setError('Failed to add JAMB topic detail.');
        setSnackbarOpen(true);
        return;
      }
      setTopicDetails([...topicDetails, created]);
      setNewDetail({ ...emptyDetail, sectionId: newDetail.sectionId });
      setMessage('JAMB topic detail added successfully!');
      setSnackbarOpen(true);
    } catch (err) {
      setError('Failed to add JAMB topic detail.');
      setSnackbarOpen(true);
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await api.delete(`/jamb-topic-details/${id}`, { withCredentials: true });
      const data = res.data;
      ...
      if (!ok) {
        setError((data && (data.error || data.message)) || 'Failed to delete JAMB topic detail.');
        setSnackbarOpen(true);
        return;
      }
      setTopicDetails(topicDetails.filter(td => (td._id || td.id) !== id));
      setMessage('JAMB topic detail deleted.');
      setSnackbarOpen(true);
    } catch (err) {
      const serverMsg = (err as any)?.response?.data?.error || (err as any)?.response?.data?.message || (err as any)?.message || 'Failed to delete JAMB topic detail.';
  ...
  const handleSave = async (id: string) => {
    ...
    try {
      ...
      const res = await api.put(`/jamb-topic-details/${id}`, payload, { withCredentials: true });
      const updated = res.data;
      if (!updated) {
        setError('Failed to update JAMB topic detail.');
        setSnackbarOpen(true);
        return;
      }
      ...
      setEditId(null);
      setEditFields(null);
      setMessage('JAMB topic detail updated!');
      setSnackbarOpen(true);
    } catch (err: any) {
      const serverMsg = err?.response?.data?.error || err?.response?.data?.message || err?.message || 'Failed to update JAMB topic detail.';
      setError(String(serverMsg));
      setSnackbarOpen(true);
    }
  };
  ...
  return (
    <Box>
      ...
      <Typography variant="h6" gutterBottom>Manage JAMB Topic Details</Typography>
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 4, borderRadius: 3 }}>
        <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>Add New JAMB Topic Detail</Typography>
        <Divider sx={{ mb: 2 }} />
        ...
        {Array.isArray(newDetail.quizzes) && newDetail.quizzes.map((quiz: any, idx: number) => (
          <Paper key={idx} elevation={0} sx={{ p: 2, mb: 2, background: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
            ...
            { /* ensure focus tracked for symbol insertion */ }
            ...
            <TextField label="Answer Index" type="number" value={quiz.answer ?? ''} onChange={e => setNewDetail((d: typeof emptyDetail) => {
              ...
              return { ...d, quizzes };
            })} size="small" sx={{ width: 140, mb: 1 }} onFocus={handleFocus} />
            <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
              {Array.isArray(quiz.explanations) && quiz.explanations.map((exp: string, eidx: number) => (
                <TextField key={eidx} label={`Explanation ${eidx + 1}`} value={exp} onChange={e => setNewDetail((d: typeof emptyDetail) => {
                  ...
                  return { ...d, quizzes };
                })} size="small" sx={{ width: 200 }} onFocus={handleFocus} />
              ))}
              <Button variant="outlined" size="small" onClick={() => setNewDetail((d: typeof emptyDetail) => {
                const quizzes = [...(d.quizzes || [])];
                const explanations = [...(quizzes[idx].explanations || [])];
        ...
        <Button onClick={handleAdd} variant="contained" color="success" sx={{ mt: 2, fontWeight: 700, fontSize: 18, borderRadius: 2, boxShadow: 2 }}>Add JAMB Topic Detail</Button>
      </Paper>
      <Paper elevation={2}>
        <List>
                ...
                <ListItem alignItems="flex-start" secondaryAction={
                  editId === (detail._id || detail.id) ? (
                    ...
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      ...
                      <Box sx={{ width: 12 }} />
                      <IconButton edge="end" color="error" onMouseDown={e => e.preventDefault()} onClick={() => confirmDelete(detail._id || detail.id || '', detail.slug || detail.description || 'this JAMB topic detail')}><DeleteIcon /></IconButton>
                    </Box>
                  )
                }>
...
export default AdminJambTopicDetailsTab;
