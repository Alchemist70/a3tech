...
const AdminJambQuestionBankTab: React.FC = () => {
  ...
  const fetchSubjects = async () => {
    try {
      // Try to fetch from JAMB sections first
      const response = await api.get('/jamb-sections');
      ...
      // Fallback to default JAMB subjects if API fails
      setSubjects([
        'Use of English',
        'Mathematics',
        'Physics',
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const response = await api.get('/question-bank/jamb', { params });
  ...
  const handleSaveQuestion = async () => {
    try {
      setLoading(true);
      if (editingId) {
        await api.put(`/question-bank/jamb/${editingId}`, formData);
      } else {
        await api.post('/question-bank/jamb', formData);
      }
      handleCloseDialog();
      // After adding a new question, reset to page 1 and set subject filter to ensure it appears
      if (!editingId) {
        setPage(1);
  ...
  const handleDeleteQuestion = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this question?')) {
      try {
        await safeDelete(`/question-bank/jamb/${id}`);
        fetchQuestions();
        setError('');
      } catch (error: any) {
                  ...
                  <Paper key={cIdx} sx={{ p: 2, backgroundColor: '#fafafa', borderLeft: '4px solid #f57c00' }}>
                    ...
                    {Array.isArray(chart.datasets) && chart.datasets.map((dataset: any, dIdx: number) => (
                      <Box key={dIdx} sx={{ mb: 1, p: 1, background: 'background.default', borderRadius: 1 }}>
                        <TextField label={`Dataset ${dIdx + 1} Label`} value={dataset.label || ''} onChange={(e) => updateDataset(cIdx, dIdx, 'label', e.target.value)} size="small" fullWidth sx={{ mb: 1 }} onFocus={handleFocus} />
                        <TextField label={`Dataset ${dIdx + 1} Data (comma-separated numbers)`} value={dataset.data ? dataset.data.join(', ') : ''} onChange={(e) => updateDataset(cIdx, dIdx, 'data', e.target.value.split(',').map((n: string) => Number(n.trim()) || 0))} size="small" fullWidth sx={{ mb: 1 }} onFocus={handleFocus} />
                        <TextField label="Background Color (hex or comma-separated per-label colors)" value={dataset.backgroundColor || ''} onChange={(e) => updateDataset(cIdx, dIdx, 'backgroundColor', e.target.value)} size="small" fullWidth sx={{ mb: 1 }} placeholder="#FF6384" onFocus={handleFocus} />
                        <TextField label="Border Color (hex or name)" value={dataset.borderColor || ''} onChange={(e) => updateDataset(cIdx, dIdx, 'borderColor', e.target.value)} size="small" fullWidth placeholder="#FF6384" onFocus={handleFocus} />
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await safeDelete(`/question-bank/jamb/${deleteId}`);
                setDeleteOpen(false);
                setDeleteId(null);
                fetchQuestions();
                setError('');
...
export default AdminJambQuestionBankTab;
