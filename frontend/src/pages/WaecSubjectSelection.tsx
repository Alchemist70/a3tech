...
import SEBPrompt from '../components/SEBPrompt';

const defaultWaecSubjects = [
  'English Language',
  'Mathematics',
  'Physics',
  'Chemistry',
...
const WaecSubjectSelection: React.FC = () => {
  ...
  const [subjects, setSubjects] = useState<string[]>(defaultWaecSubjects);
  ...
  useEffect(() => {
    ...
    // Fetch WAEC sections for subjects list
    const fetchSections = async () => {
      try {
        const resp = await api.get('/waec-sections');
        if (Array.isArray(resp.data)) {
          const names = resp.data.map((s: any) => s.name).filter(Boolean);
          if (names.length) setSubjects(names);
        }
      } catch (e) {
        console.warn('Failed to fetch WAEC sections', e && (e as any).message ? (e as any).message : e);
      }
    };
  ...
  // Persist current selection to sessionStorage so confirmation/instruction pages can show it immediately
  useEffect(() => {
  ...
  const handleSubjectToggle = (subject: string) => {
    ...
    if (subject === 'English Language' || subject === 'Mathematics') return; // Can't unselect English or Math
  ...
  const handleNext = async () => {
    if (selectedSubjects.length < 7 || selectedSubjects.length > 9) {
      setError('Please select between 7 and 9 subjects (English Language + Mathematics compulsory)');
      return;
    }
    ...
    try {
      ...
      if (isComboLocked) {
        navigate(`/mock-test/waec/confirm/${mockTestId}`);
      } else {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        console.error('[WaecSubjectSelection] Error saving subjects:', errorMsg);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmation = async () => {
    // Subjects already saved in handleNext; just navigate
    navigate(`/mock-test/waec/confirm/${mockTestId}`);
  };
      ...
      <Container maxWidth="md">
        <Box sx={{ mb: 6 }}>
          ...
          <Typography variant="h4" gutterBottom sx={{ fontWeight: 800, color: 'primary.dark' }}>
            WAEC Mock Test - Subject Selection
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ lineHeight: 1.7 }}>
            Choose 5-7 additional subjects to accompany the compulsory English Language and Mathematics (minimum 7 total, maximum 9). You can only change your subject combination once every 8 months.
          </Typography>
          ...
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
              ...
              {subjects.map((subject) => (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: 1.5,
                    cursor: subject === 'English Language' ? 'not-allowed' : 'pointer',
                    background: selectedSubjects.includes(subject)
                      ? alpha(theme.palette.primary.main, 0.1)
                      : 'transparent',
                    transition: 'all 0.3s ease',
                    opacity: subject === 'English Language' ? 1 : selectedSubjects.includes(subject) ? 1 : 0.7,
                    '&:hover': {
                      borderColor: subject === 'English Language' ? 'divider' : 'primary.main',
                      background:
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Checkbox
                      ...
                      disabled={subject === 'English Language' || subject === 'Mathematics' || isComboLocked}
                      sx={{ ml: -1 }}
                    />
                    <Box>
            ...
            <Box sx={{ mb: 4, p: 2, background: alpha(theme.palette.info.main, 0.1), borderRadius: 1.5, border: `1px solid ${alpha(theme.palette.info.main, 0.3)}` }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1 }}>
                Selected Subjects ({selectedSubjects.length}/7-9)
              </Typography>
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {selectedSubjects.map((subject) => (
                  <Chip
                    ...
                    label={subject}
                    color="primary"
                    variant={subject === 'English Language' ? 'filled' : 'outlined'}
                    onDelete={!isComboLocked && subject !== 'English Language' && subject !== 'Mathematics' ? () => handleSubjectToggle(subject) : undefined}
                  />
                ))}
              </Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
              ...
              <Button
                ...
                onClick={handleNext}
                disabled={selectedSubjects.length < 7 || selectedSubjects.length > 9}
              >
                Next
              </Button>
            </Box>
        ...
        <Box sx={{ mt: 2 }}>
          <SEBPrompt examUrl={`${window.location.origin}/mock-test/waec/test/${mockTestId}`} />
        </Box>
      </Container>
      ...
      <Dialog open={showConfirmation} onClose={() => setShowConfirmation(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2" sx={{ fontWeight: 600, mb: 1 }}>Compulsory Subjects:</Typography>
            <Typography variant="body2">• English Language</Typography>
          ...
          <Box sx={{ p: 2, background: alpha(theme.palette.primary.main, 0.1), borderRadius: 1.5, mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {selectedSubjects.map((subject) => (
                <Chip key={subject} label={subject} color="primary" variant={subject === 'English Language' || subject === 'Mathematics' ? 'filled' : 'outlined'} />
              ))}
            </Box>
          </Box>
...
export default WaecSubjectSelection;
