...
import { ArrowForward, ArrowBack, CheckCircle, ContentCopy } from '@mui/icons-material';
import api from '../api';
...
const WaecConfirmation: React.FC = () => {
  ...
  const handleProceed = () => {
    navigate(`/mock-test/waec/instructions/${mockTestId}`);
  };
    ...
    <Box sx={{ minHeight: '100vh', background: theme.palette.background.default, py: { xs: 4, md: 8 } }}>
      <Container maxWidth="md">
        <Box sx={{ mb: 6 }}>
          <Button
            startIcon={<ArrowBack />}
            onClick={() => navigate(`/mock-test/waec/subjects/${mockTestId}`)}
...
export default WaecConfirmation;
