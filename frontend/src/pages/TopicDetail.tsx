...
import { Box, Typography, Breadcrumbs, Link, CircularProgress, Paper, Divider, Chip, Tabs, Tab, Button, useTheme } from '@mui/material';
...
// Small helper component to render videos from multiple sources safely.
const VideoPlayer: React.FC<{ src: string }> = ({ src }) => {
  ...
  const isDirect = /\.(mp4|webm|ogg|m4v|mov)$/i.test(src) || src.startsWith('blob:') || src.startsWith('data:');

  // HLS and direct video handling
  useEffect(() => {
  ...
  // Iframe embed detection: attempt to load iframe and mark success/failure via onLoad/timeout
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      if (mounted && iframeStatus === 'loading') {
  ...
  const onIframeLoad = () => {
    // when iframe fires load we mark it as loaded
    setIframeStatus('loaded');
  };

  const commonIframe = (iframeSrc: string, title = 'Video') => (
    <Box>
      <iframe ref={iframeRef} width="100%" height="270" src={iframeSrc} title={title} frameBorder="0" allowFullScreen style={{ borderRadius: 8 }} onLoad={onIframeLoad} />
      <Box sx={{ mt: 1 }}>
        <Button variant="outlined" size="small" href={iframeSrc} target="_blank" rel="noopener noreferrer">Open in new tab</Button>
      </Box>
    </Box>
  );

  // YouTube handling: attempt embed first; fall back to inline thumbnail + retry/fallback UI
  if (isYouTube) {
    // extract video id
    let videoIdMatch = src.match(/(?:v=|\/)([0-9A-Za-z_-]{11})/);
    ...
    const embedUrl = videoId ? `https://www.youtube.com/embed/${videoId}?rel=0&enablejsapi=1` : src;
    ...
    // If iframe load failed, show thumbnail with retry button that will attempt embed again
    if (iframeStatus === 'failed') {
      return (
        <Box sx={{ maxWidth: 480, width: '100%' }}>
          {thumb && (
            <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden' }}>
              <Box component="img" src={thumb} alt="YouTube thumbnail" sx={{ width: '100%', display: 'block' }} />
          ...
          <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
            <Button variant="outlined" size="small" onClick={() => {
              // retry embedding by resetting iframe status and forcing re-render
              setIframeStatus('loading');
              // small delay to allow iframe to mount again
              setTimeout(() => setIframeStatus('loading'), 10);
            }}>Retry embed</Button>
    ...
    // If loading or loaded, render iframe (it will call onLoad if successful)
    return commonIframe(embedUrl, 'YouTube video');
  }
  ...
  if (isVimeo) {
    ...
    return commonIframe(embedUrl, 'Vimeo video');
  }
        ...
        {hlsError && (
          <Box sx={{ mt: 1 }}>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
              Unable to play HLS stream in this browser. You can open it in a new tab.
            </Typography>
            <Button variant="outlined" size="small" href={src} target="_blank" rel="noopener noreferrer">Open HLS in new tab</Button>
          </Box>
        )}
      </Box>
    );
  ...
  // Direct video files or blobs/data URLs
  if (isDirect) {
    return <video ref={videoRef} controls style={{ width: '100%', borderRadius: 8 }} />;
  }

  // Fallback: try iframe embedding for other URLs
  return commonIframe(src, 'Video');
};
